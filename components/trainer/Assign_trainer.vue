...
export default {
  ...
  methods: {
    ...
    async fetchClients() {
      ...
      try {
        const response = await fetch(`/api/coaches/${this.coachId}/clients`);
        if (!response.ok) {
    ...
    async fetchWorkouts() {
      ...
      try {
        const response = await fetch(`/api/coaches/${this.coachId}/workouts`);
        if (!response.ok) {
    ...
    async assignTraining() {
      ...
      try {
        ...
        const response = await fetch(`/api/coaches/${this.coachId}/assign-training`, {
          method: 'POST',
